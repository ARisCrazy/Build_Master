...
<body>
    <div class="container">
        <div class="products">
            ...
            $sql = "SELECT * FROM prebuilt";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<div class='product'>";
                    echo "<img src='" . $row['photo'] . "' alt='" . $row['prdname'] . "'>";
                    echo "<h3>" . $row['prdname'] . "</h3>";
                    echo "<p>" . $row['prddesc'] . "</p>";
                    echo "<br>";
                    echo "<p>Price: ₹" . $row['prdprice'] . "</p>";
                    echo "<form action='add_to_prd.php' method='post'>
                          <input type='hidden' name='prdid' value='" . $row['prdid'] . "'>
                          <input type='hidden' name='prdname' value='" . $row['prdname'] . "'>
